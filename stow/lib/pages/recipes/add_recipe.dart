        ...
        child: Padding(
          ...
          child: SingleChildScrollView(
            ...
            children: [
              // name textfield
              SizedBox(height: 20,),
              Text('Name of Recipe', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16),),
              Padding(
              ...
              SizedBox(height: 20,),
              Text('Prep Time', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16),),
              Padding(
                padding: const EdgeInsets.only(right: 32.0),
                child: TextFormField(
              ...
              SizedBox(height: 20,),
              Text('Cook Time', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16),),
              Padding(
                padding: const EdgeInsets.only(right: 32.0),
                child: TextFormField(
              ...
              SizedBox(height: 20,),
              Text('Instructions', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16),),
              ..._getInstructions(),
              SizedBox(height: 40,),
              Text('Ingredients', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16),),
              ..._getIngredients(),
              SizedBox(height: 40,),
              ElevatedButton(
                onPressed: (){
                  if (_formKey.currentState!.validate()) {
                    final recipe = 
                      ...
                      Recipe(recipeId: uuid.v4(), name: nameController.text,
                        instructions: instructionsList, userId: authBloc.state.user!.uid,
                        ingredients: ingredientsList, cookTimeMin: int.parse(cookController.text),
  ...
  Widget _addRemoveButtonInstructions(bool add, int index){
    return InkWell(
      onTap: (){
        ...
        else instructionsList.removeAt(index);
        setState((){});
      },
      child: Container(
        width: 30,
...
class _InstructionsTextFieldsState extends State<InstructionsTextFields> {
  ...
  Widget build(BuildContext context) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      instructionsController.text = _AddRecipePageState.instructionsList[widget.index] ?? '';
    });
    ...
    return TextFormField(
      ...
      decoration: InputDecoration(
        hintText: 'Enter the step\'s instructions'
      ),
      validator: (String? value) {
        if (value == null || value.isEmpty) {
          return 'Please enter some text';
...
class _IngredientsTextFieldsState extends State<IngredientsTextFields> {
  ...
  Widget build(BuildContext context) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      ingredinetsController.text = _AddRecipePageState.ingredientsList[widget.index] ?? '';
    });
